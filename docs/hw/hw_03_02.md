...
![fig-1-1](hw_03_02.assets/fig-1-1.png){ : style="height:180px"}

#### (2) 所有由 $a$ 和 $b$ 组成且不含子串 $abb$ 的串

![fig-1-2](hw_03_02.assets/fig-1-2.png){ : style="height:160px"}

## 练习 3.7.2 (1)

用算法 3.22 模拟图 3-29 中的 NFA 在处理输入 $aabb$ 时的过程.

![fig3-29](hw_03_02.assets/fig3-29.png){ : style="height:180px"}
...
```c
S = epsilon-closure(s_0); // 初始化: 起始状态的 ε-闭包
c = nextChar();
while (c != eof) {
    S = epsilon-closure(move(S,c)); // 对输入进行 ε-迁移
    c = nextChar();
}
// 判断接受与否
...
> 算法 3.23:
> (图片缺失) 每一个"单位正则式"对应的 NFA

![FSMs-373-1-origin](hw_03_02.assets/FSMs-373-1-origin.png){ : style="height:230px"}

使用算法 3.20 (子集构造法) 将 NFA 转换为 DFA:

> 算法 3.20 **子集构造法**: 从 "仅经过任意个 ε 能到达的集合" 开始, 迭代地寻找比该集合 **多** 经过一个终结符能到达的集合.
...
![FSMs-373-1-simple](hw_03_02.assets/FSMs-373-1-simple.png){ : style="height:200px"}

#### (4) $(a|b)^{*}abb(a|b)^{*}$

使用算法 3.23 将正则表达式转换为 NFA:

![FSMs-373-2-origin](hw_03_02.assets/FSMs-373-2-origin.png){ : style="height:400px"}
...
![FSMs-373-2-simple](hw_03_02.assets/FSMs-373-2-simple.png){ : style="height:250px"}
